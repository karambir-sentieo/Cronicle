...
module.exports = Class.create({
		...
		function(err) {
			...
			for (var idx = 0, len = servers.length; idx < len; idx++) {
				...
				if (hook_text_templates[hook_data.action]) {
					...
					let is_send_log = hook_action!='job_start' || !this.server.config.get('web_hook_only_error_logs');
	...
	finishJob: function(job) {
		...
		if (hook_text_templates[hook_action]) {
			...
			let is_send_log = hook_action!='job_complete' || !this.server.config.get('web_hook_only_error_logs');
