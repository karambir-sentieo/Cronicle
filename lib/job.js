...
module.exports = Class.create({
		...
		function(err) {
			...
			for (var idx = 0, len = servers.length; idx < len; idx++) {
				...
				if (hook_text_templates[hook_data.action]) {
					...
					let is_send_log = hook_data.action!='job_start' || !self.server.config.get('web_hook_only_error_logs');
					
					if (job.web_hook && is_send_log) {
					...
					if (self.server.config.get('universal_web_hook') && is_send_log) {
	...
	finishJob: function(job) {
		...
		if (hook_text_templates[hook_action]) {
			...
			let is_send_log = hook_action!='job_complete' || !self.server.config.get('web_hook_only_error_logs');
			...
			if (this.server.config.get('universal_web_hook') && is_send_log) {
